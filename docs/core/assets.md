...
That's from [examples/basic](example-basic).
...
So that `import x from './x.png'` is typed (like `vite/client`), the
...
`include` already covers — no manual `reference` needed. It declares, per
managed extension, the bare module plus each query form:
...
The extension list and `defaultMode` default from the general `compiler.assets`,
so the typings match what the plugin emits; `assetsTypes.extensions` can
override the extension list for the typings only.
...
Point0 keeps no `Content-Type` table — the dev route hands the file to
...
These come from Bun's plugin model:
