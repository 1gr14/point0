...
title: Plugin
description:
  A plugin is a bundle of methods you define once and inject into any point's
  chain with .use().
---
...
Any point can now `.use(mePlugin)` and read `me` — from `ctx` in its loader,
...
The explicit equivalent is `Point0.lets('plugin', 'me')` — the second argument
...
any of those on a plugin is a type error. It only carries methods that _merge
into_ the point that uses it.
...
- **`ctx`** — appended to the consumer's `ctx` (visible in its loader and later
  `.ctx`).
- **`props`** — props the plugin's `.with` returns merge into the consumer's
  `props`.
...
- **Related queries**, event subscriptions, wrappers, tags, and description —
  all fold in.
...
Nesting is the exception in one direction: when a plugin does
`.use(otherPlugin)`, the inner plugin's `ctx`/`props` _are_ visible to the outer
...
See [`.with`](with) and [ctx](ctx) for each side in depth.
...
- `.ctx` **throws** the error; `.with` **returns** it. A returned `Error` from
  `.with` short-circuits to the error component — see
  [Loading & error](loading-error).
- Both branches `return { me }` on success. That narrows `me` to non-null for
  every point that uses the plugin, so consumers never re-check it.
...
Same shape, but return a [`redirect`](navigation) instead of throwing —
...
A plugin can declare a [`.relatedQuery`](query); it merges into whatever
...
contributes methods; it never _is_ a mountable or routable point.
...
type rejects a widening or a kind clash.
