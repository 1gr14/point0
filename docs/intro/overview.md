...
| ------------------------ | --------------------------------------------------------------------- | -----------: | ----------: | ----------: |
| `@point0/core`           | the framework itself, without the socket feature                      |     223.4 KB |     57.1 KB |     48.6 KB |
| `@point0/core` + sockets | optional — the same core with sockets on (`server: { socket: true }`) |     304.2 KB |     75.9 KB |     63.9 KB |
| `@point0/react-dom`      | React/DOM bindings — `mount` and the router                           |      14.0 KB |      5.2 KB |      4.6 KB |
