...
bring. Sockets are also **optional in the literal sense**: an app that never
turns `socket` on never downloads them — the compiler cuts the whole socket
runtime out of the client bundle. Full model on the [Channel](socket) and
...
| ------------------------ | --------------------------------------------------------------------- | -----------: | ----------: | ----------: |
| `@point0/core`           | the framework itself, without the socket feature                      |     223.1 KB |     56.6 KB |     48.5 KB |
| `@point0/core` + sockets | optional — the same core with sockets on (`server: { socket: true }`) |     303.6 KB |     75.1 KB |     63.5 KB |
| `@point0/react-dom`      | React/DOM bindings — `mount` and the router                           |      14.0 KB |      5.1 KB |      4.6 KB |
| `@1gr14/route0`          | peer — typed routes and URL building                                  |      43.4 KB |     12.7 KB |     11.5 KB |
| `@tanstack/react-query`  | peer — the cache every loader rides on                                |      47.0 KB |     13.7 KB |     12.5 KB |
| `wouter`                 | peer — history and route matching                                     |       5.6 KB |      2.7 KB |      2.4 KB |
| `unhead`                 | peer — the `<head>`                                                   |      20.2 KB |      7.8 KB |      7.1 KB |
| `@1gr14/error0`          | optional peer — typed errors across the wire                          |      10.8 KB |      3.1 KB |      2.8 KB |
| **total**                | everything above, minus the optional rows                             | **361.4 KB** | **99.7 KB** | **85.2 KB** |
