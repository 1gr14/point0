/**
 * The slow integration test files — single source of truth, shared by the test runner
 * (scripts/test-parallel.ts) and the CI matrix builder (scripts/ci-decide.ts).
 *
 * "Slow" = heavy integration files that boot a full dev/build tree. They run strictly one
 * file per process (never a combined `bun test`) so module-level state and ports can't bleed
 * across them, and in CI each one gets its own runner (one shard per file). Paths are relative
 * to `packages/`.
 */
