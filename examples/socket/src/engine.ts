...
export const engine = Engine.create({
  ...
  server: {
    scope: 'root',
    // the bare WebSocket endpoint the channels multiplex over — off by default, sockets need it on
    socket: true,
    // one process needs no backplane; behind a load balancer add one, e.g.: backplane: process.env.REDIS_URL
