...
import { getPreloadManifestPathSegments, type PreloadManifest } from '../src/preload-manifest.js'
  ...
  describe.each(bundlers)('%s', (bundler) => {
    it(
      ...
      wrp({ ssr: true, vite: bundler === 'vite' }, async ({ tp, engine }) => {
        ...
        const manifestPath = tp.resolve('dist', 'client', ...getPreloadManifestPathSegments('root'))
