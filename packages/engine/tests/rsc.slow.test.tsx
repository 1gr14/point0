...
// RSC e2e — a real dev server + a real Chromium, then a real production build. Covers what the
// in-process harness (rsc.fast.test.tsx) cannot: hydration of SSR-shipped element payloads (zero
// refetch, zero hydration errors), interactive islands, `.clientOnly()` islands, a component
// point living in its OWN FILE (aggregator lazy record → its own chunk, loaded on demand), and
// the compiler strip guarantees (server code out of the client bundle, client-only render out of
// the server bundle).
//
