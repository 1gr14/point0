...
describe('fetch', () => {
  ...
  it(
    'state new each server request, but cache shared between requests',
    async () => {
  ...
  it(
    'cookies used cookies of first request, and do not propagate them when ssr request another points',
    async () => {
      const root = createRoot()
      const page = root
        .lets('page', 'home', '/')
  ...
  it(
    'can fetch point with engine.withFetch',
    async () => {
      const root = createRoot()
      const query = root
        .lets('query', 'test')
